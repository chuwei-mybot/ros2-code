...
class Set_Model_State(Node):
    ...
    def __init__(self,name):
        ...
        self.state_pub = self.create_publisher(ModelState,'tribot2/gazebo/set_model_state',10)
        #发布跟随者速度话题
        self.cmd_vel_pub = self.create_publisher(Twist, 'tribot2/cmd_vel',10)
        # subscribe control input
        self.rate_sub = self.create_subscription (Twist,'tribot2/cmd_vel',self.MotionCallback,10)
        self.state_sub = self.create_subscription(ModelState,"tribot2/gazebo/set_model_state", self.broadcast_odom_tf,10)
        self.timer = self.create_timer(0.001,self.on_timer) #创建固定周期计时器，控制跟随者运动
    ...
    def broadcast_odom_tf(self,model_state):
        ...
        transform.child_frame_id = 'tribot2/base_footprint'                   # 设置一个坐标变换的目标坐标系
...
# main function
    ...
    def on_timer(self):
        ...
        msg = Twist()                                      # 创建速度控制消息
        scale_rotation_rate = 0.5                          # 根据tribot角度，计算角速度
        self.get_logger().info(
                'X='+str(
        ...
        scale_forward_speed = 0.4                         # 根据海龟距离，计算线速度
    ...
    def MotionCallback(self,cmd_vel):
        ...
        if (self.last_t==0):
            ...
            initial_state_pub = self.Generate_State(initial_state,"tribot2")
            ...
            state_t_pub=self.Generate_State(self.state,"tribot2")
...
def main(args=None):                                 # ROS2节点主入口main函数
    ...
    node = Set_Model_State("follower_kinematic2")        # 创建ROS2节点对象并进行初始化
